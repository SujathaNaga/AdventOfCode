...
input2="""dac abcf ac fdbcga dgcbae gcbfde fgcbd agfed adcgf cdbgfea | cbfa bcafdg cbfa bafcgd
...
input3="""acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf"""
...
def funcb(lines):
    signal_to_number={'cf':1, 'acdeg':2, 'acdfg':3, 'bcdf':4, 'abdfg':5, 'abdefg':6, 'acf':7, 'abcdefg':8, 'abcdfg':9}
    len_to_number={2:1, 4:4, 3:7, 7:8}

    final_sum=0

    for line in lines:
        narrowed_down_mappings={}
        signal,output=line.split(' | ')
        output=output.split(' ')
        signal=signal.split(' ')
        signal_decode_list=[v for v in signal if len(v) in [2,3,4,7]] # has only 2/3/4 signals from input 
        signal_decode_list.sort(key=lambda x:len(x), reverse=True) # sort from 4 -> 2 len
        signal_decode_list=[list(e) for e in signal_decode_list]

        # narrow down
        for signal_list in signal_decode_list:
            for s in signal_list:
                number=len_to_number[len(signal_list)]
                key=get_key(signal_to_number, number)
                if s not in narrowed_down_mappings:
                    narrowed_down_mappings[s]=list(key)
                else:
                    existing_list=narrowed_down_mappings[s]
                    narrowed_down_mappings[s]=[a for a in existing_list if a in list(key)]
        # print(narrowed_down_mappings)

        # special case of 2 and 1
        loop=True
        while loop:
            letter_tuple_to_len=defaultdict(lambda:0)
            new_narrowed_down_mappings=copy.deepcopy(narrowed_down_mappings)
            for k,v in new_narrowed_down_mappings.items():
                if len(v) in [1,2]:
                    letter_tuple_to_len[tuple(v)]+=1
            
            for k,v in letter_tuple_to_len.items():
                for letter,possibilities in new_narrowed_down_mappings.items():
                    if len(possibilities) > len(list(k)):
                        new_narrowed_down_mappings[letter]=[x for x in possibilities if x not in list(k)]
            
            if new_narrowed_down_mappings == narrowed_down_mappings:
                loop=False
            narrowed_down_mappings=copy.deepcopy(new_narrowed_down_mappings)

        # print(narrowed_down_mappings)

        # club 2 together
        for k,v in new_narrowed_down_mappings.items():
            keys=get_keys_from_value(new_narrowed_down_mappings,v)
            keys.sort()
            key=''.join(keys)

            if key not in narrowed_down_mappings:
                narrowed_down_mappings[key]=v

        # print(narrowed_down_mappings)
        # process output

        found_input_to_correct_number={} # 'string':'numberstring'
        final_str=''
        for output_str in output:
            original_output_str=''.join(sorted(copy.deepcopy(output_str)))
            # print('key',output_str)
            if len(output_str) in len_to_number:
                value=str(len_to_number[len(output_str)])
                found_input_to_correct_number[original_output_str]=value
                # print('found unique', output_str, found_input_to_correct_number[original_output_str])
            elif original_output_str not in found_input_to_correct_number:
                # deduce from ones. 
                len_output_str=len(output_str)
                mapped_output=''
                sures_ones=get_keys_from_len_of_value(narrowed_down_mappings,1)

                for s in sures_ones:
                    if s in output_str:
                        mapped_output+=''.join(narrowed_down_mappings[s])
                        mapped_output=''.join(sorted(mapped_output)) # str -> sort -> list -> str
                        output_str.replace(s,'')

                sures_twos=get_keys_from_len_of_key(narrowed_down_mappings,2)
                for s in sures_twos:
                    if has_all_chars(output_str, s):
                        mapped_output+=''.join(narrowed_down_mappings[s])
                        mapped_output=''.join(sorted(mapped_output)) # str -> sort -> list -> str
                        output_str.replace(s,'')
                
                possibilities=get_keys_from_len_of_key(signal_to_number,len_output_str)
                # print('mapped',mapped_output)
                p2=[p for p in possibilities if has_all_chars(p, mapped_output)]
                # print(p2, possibilities)
                if len(p2)==1:
                    # only one possibility
                    signal=p2[0]
                    found_input_to_correct_number[original_output_str]=str(signal_to_number[signal])
                else:
                    # print('sure ones', sures_ones)
                    # print('sure twos', sures_twos)
                    # print(narrowed_down_mappings)
                    # print(p2, possibilities, mapped_output, '-orig:',original_output_str, output)
                    raise ErrorException
            
            final_str+=found_input_to_correct_number[original_output_str]
            # print(final_str)
