#include "common.h"
using namespace std;
const char* day2_2017_input = R"(3093 749 3469 142 2049 3537 1596 3035 2424 3982 3290 125 249 131 118 3138
...
void day2_2017()
{
	string copydata = day2_2017_input;
	vector<string> tokens = SplitString(copydata, '\n');
	int total = 0;
	for (auto& token : tokens)
	{
		vector<string> numbers = SplitString(token, ' ');
		vector<int> nums;
		for (const auto& n : numbers)
		{
			nums.push_back(atoi(n.c_str()));
		}
		total += (*max_element(nums.begin(), nums.end()) - *min_element(nums.begin(), nums.end()));
	}
	cout << total << endl;
