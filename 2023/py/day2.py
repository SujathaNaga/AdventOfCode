...
input="""Game 1: 4 red, 1 green, 15 blue; 6 green, 2 red, 10 blue; 7 blue, 6 green, 4 red; 12 blue, 10 green, 3 red
...
def day2():
    ...
    for line in lines:
        ...
        if good:
            good_games_sum+=game_id
    
    print(good_games_sum)    
    ...
    for line in lines:
        ...
        for a in sets:            
            ...
            for event in a.split(', '):
                parts=event.split()
                cubes[parts[1]] = int(parts[0])            
            max_red_line=cubes.get('red',0) if cubes.get('red',0)>max_red_line else max_red_line
            max_green_line=cubes.get('green',0) if cubes.get('green',0)>max_green_line else max_green_line
            max_blue_line=cubes.get('blue',0) if cubes.get('blue',0)>max_blue_line else max_blue_line
        
        power+=max_green_line*max_red_line*max_blue_line
