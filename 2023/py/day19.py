...
for line in parts[0].split('\n'):
    ...
    for condition in groups[1].split(','):
        try:
            rules_dict[groups[0]].append(condition.split(':'))
        except ValueError:
            rules_dict[groups[0]].append([condition])

    
def recursive_eval(input_dict, label):
    if label == 'A':
        return True
    if label=='R':
        return False    
    
    for condition in rules_dict[label]:
        if len(condition) == 2:
            if eval(condition[0],input_dict):
                return recursive_eval(input_dict, condition[1])
        else:
            return recursive_eval(input_dict, condition[0])
...
def day19():
    ...
    for line in parts[1].split('\n'):
        ...
        for part in parts:
            ...
            input_dict[var]=int(value)
            v+=int(value)
        
        if recursive_eval(input_dict, "in"):
            ans+=v
    
    print(ans)           

total_b=0
def calculate_range(input_conditions):
    ...
    for condition in input_conditions:
        letter=condition[0]
        values_dict[letter] = [i for i in values_dict[letter] if eval(condition, {letter:i})]
    
    total_range=math.prod([len(v) for k,v in values_dict.items()])
    return total_range
    
def recursive_process_b(input_conditions, label):
    global total_b
    if label == 'A':
        return True
    if label=='R':
    ...
    for condition in rules_dict[label]:
        if len(condition) == 2:         
            if recursive_process_b(input_conditions+[condition[0]], condition[1]):
                total_b += calculate_range(input_conditions+[condition[0]])
        else:            
            if recursive_process_b(input_conditions, condition[0]):
                total_b +=  calculate_range(input_conditions)
            return
    
        #  false case
        if condition[0][1]=='<':
            new_condition=condition[0][0]+'>='+condition[0][2:]
            input_conditions.append(new_condition)
        elif condition[0][1]=='>':
            new_condition=condition[0][0]+'<='+condition[0][2:]
            input_conditions.append(new_condition)
        
def day19_b():
    recursive_process_b([],"in")
    print(total_b)
