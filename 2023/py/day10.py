...
input="""F.F77.F7F7FFJJ..77FF.--L777.-7777--F7F7FJFFF7FF|.F77FF|-7-|-F7-7-FF|FJ-FFJ7-F--F|-L--7L|-7FFFF---7-FJ.7J.F-7J.F|7-LF7L7|.F-L|7...LF--LL7F-77
...
# prep data
lines=[list(l) for l in input.split('\n')]
...
def find_next_cells(cell_and_parent):
    global processed    
    new_cell_and_parents=[]
        
    # if end
    if len(cell_and_parent) > 1 and len(set([c[0] for c in cell_and_parent]))==1:
        processed[cell_and_parent[0][0]]=cell_and_parent[0][2]+1
        final_index_a=cell_and_parent[0][2]+1
        return new_cell_and_parents, final_index_a
        
    
    for cell in cell_and_parent:
        cell,parent,parent_index=cell[0],cell[1],cell[2]
        
        c=lines[cell[0]][cell[1]]
        
        (first,second)=directions[c]
        
        first=(first[0]+cell[0],first[1]+cell[1])
        second=(second[0]+cell[0],second[1]+cell[1])
        
        index=parent_index+1
        processed[cell]=index
        
        if parent:
            nextcell=second if first==parent else first            
            new_cell_and_parents.append((nextcell, cell, index))
        else:
            new_cell_and_parents.append((first, cell, index))
            new_cell_and_parents.append((second, cell, index))
    
    return new_cell_and_parents,None
        
        
def day8():
    start_time=time()
    
    # do not recurse. reached max recursion depth
    nextlist=[(START_LOC, None, -1)]
    while True:
        nextlist,final_index_a=find_next_cells(nextlist)
        if final_index_a:   
            print('a',final_index_a)
            break            
    
    # followed idea from https://www.reddit.com/r/adventofcode/comments/18ez5jb/2023_day_10_part_2_shortcut_solution_using_shape/
    # from the visualizations in reddit by other users who got the solution, it appears that the inside loop tiles are exactly in the center, and about a quarter of distance from the corners
    
    row_calc=int(len(lines)/4)
    col_calc=int(len(lines[0])/4)
    total=0
    for row in range(row_calc, len(lines)-row_calc):
        for col in range(col_calc, len(lines[0])-col_calc):
