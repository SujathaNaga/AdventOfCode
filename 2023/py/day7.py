...
def find_card_type(card, puzzle_type):
    ...
    if 4 in duplicate_map.values():
        return FOUR_ORDER
    if 3 in duplicate_map.values() and 2 in duplicate_map.values():
        return FULL_HOUSE
    if 3 in duplicate_map.values() and 1 in duplicate_map.values():
        return THREE
    if list(duplicate_map.values()).count(2) == 2:
        return TWO_PAIR
    if list(duplicate_map.values()).count(2) == 1:
        return ONE_PAIR
    if list(duplicate_map.values()).count(1) == 5:
        return HIGH_CARD
    else:
