...
def check_part_number(s, lines, max_col):
    for t in s:
        if t[0]>=0 and t[0]<len(lines) and t[1]>=0 and t[1]<max_col:
            if not lines[t[0]][t[1]].isdigit() and lines[t[0]][t[1]] != '.':
                return True    
    return False
...
def day3():
    ...
    for row,line in enumerate(lines):
        number=''
        index_search_set=set()
        for col,c in enumerate(line):
            if c.isdigit():
                number+=c
                index_search_set.update({(row,col-1),(row-1,col-1),(row+1,col-1),(row-1,col),(row+1,col),(row,col+1),(row-1,col+1),(row+1,col+1)})
            else:
                if check_part_number(index_search_set, lines, len(line)):
                    important_numbers.append(int(number))
    ...
    print(sum(important_numbers))
    print('elapsed',(time()-start_time))
    
def check_gear(s, lines, max_col):
    for t in s:
        if t[0]>=0 and t[0]<len(lines) and t[1]>=0 and t[1]<max_col:            
            if lines[t[0]][t[1]]=='*':
                return (t[0],t[1])    
    ...
    for row,line in enumerate(lines):
        ...
        for col,c in enumerate(line):
            if c.isdigit():
                ...
                star_loc=check_gear(index_search_set, lines, len(line))
                if star_loc:
                    gears[star_loc].append(int(number))                    
                number=''
                index_search_set=set()          
