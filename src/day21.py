...
g_ing_list=[]
g_all_ing_dict=defaultdict(lambda: set())

for l in lines:
    
    alllist=l.split(" (contains ")
      
    ingredients,allerg=l.split(" (contains ")
    
    g_ing_list.append(ingredients.split())
    
    allerg=allerg.replace(')','')
    
    allerg=[a.strip() for a  in allerg.split(',')]
    
    for a in allerg:
        # find the intersection of existing list with the new list of ingredients for this allergent so that we can narrow down
        g_all_ing_dict[a] = g_all_ing_dict[a].intersection(set(ingredients.split())) if len(g_all_ing_dict[a]) > 0 else set(ingredients.split())

# if only one ingredient is present for an allergent, remove that ingredient from other allergents
# while until all the allergents have only one ingredient
iterate=True
while iterate:
    for a,i in g_all_ing_dict.items():
        if len(i)==1:
            #found
            value=next(iter(i))
        
            for a2,i2 in g_all_ing_dict.items():
                if a2!=a:
                    i2.discard(value)
    iterate=False
    for a,i in g_all_ing_dict.items():
...
for a,i in g_all_ing_dict.items():
    if len(i)==1:
        value=next(iter(i))
        #remove the allergent-ingredient from the list of ingredients
        for l in g_ing_list:
            try:
                l.remove(value)
            except ValueError:
